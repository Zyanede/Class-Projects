...
public class Slot {
	...
	int swordConstant;
	//selected is used in Shelves class so the shelves know which sword was seleced
	static int selected;
	...
	public Slot(final int constant) {
		...
		slot = new JButton(newSword.getImage());
		...
		slot.addMouseListener(new MouseAdapter() {
			...
			public void mouseReleased(MouseEvent e) {
				...
				//When not true, you are looking at your stock that you can place on the shelves
				if (GUIActivity.buying == false) {
					if (SwingUtilities.isLeftMouseButton(e)) {
						
						// get the actual sword that needs to beplaced
						GUIActivity.newSword = newSword; 
						//the selected variable equals constant, again to be used in shelves
						selected = constant;
					}
				}
				
				//When the buying is true, you are looking at slots that you can buy
				// this allows you to buy the swords and places them in the stock
					...
					if (GUIActivity.income.getMoney() >= newSword.getCost()) {
						...
						GUIActivity.bank.setText("Total money: $" + GUIActivity.income.getMoney());
			...
			public void mouseEntered(MouseEvent arg) {
				...
				GUIActivity.hoverStock.setText("Stock: "  + SwordStock.stock[constant]);
